...
void PullSig1300Cheby2()
{
  ...
  TFile* inputFile =new TFile("SkimFileMass_SoftDrop.root", "READ");
    //BKg Shapes:
 ...
 ZJetsTree->Draw("PrunedMass1>>PrunedMassZBkg", "(MET>300 && HT>370 && JetPt1>200 && JetPt2>170 && PrunedMass1>40 && PrunedMass1<200 && PrunedMass2>70 && PrunedMass2<100)*Evtweight*(80/35.9)");
  ...
  RooRealVar a1("a1","a1",-0.9654);
  RooRealVar a2("a2","a2",0.122);
 ...
 RooFitResult *r =  bkg.fitTo(MC_bkg,Range("Sideband1,Sideband2"),Save());
  ...
  RooRealVar chebyfrac("chebyfrac","fraction of cheby",0.5,0.,1.);
  
  // RooAddPdf Mfinal1 ("Mfinal1","signal1",RooArgList(cheby,Mvoigt),chebyfrac);
  ...
  // Mfinal1.plotOn(frame0,LineColor(kYellow));
  ...
  // sig.plotOn(frame,Components(cheby1sig),LineColor(kOrange));
   ...
   //Mfinal1.paramOn(frame0,LineColor(kYellow));
   

   // RooNLLVar nll("nll","nll",Mfinal,*toySample) ;
   RooPlot* frame5 = jetMass.frame();
